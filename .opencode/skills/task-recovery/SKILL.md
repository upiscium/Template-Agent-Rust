...
4. For leaf recovery, select only an existing `in-flight` or `failed` Work Unit from recovery status and load it with `work-unit-status`. Next call `recovery-route`; only after it returns a non-BLOCKED selection may the orchestrator delegate the stored role/objective unchanged to that exact selected agent. For Spark-unavailable families, route directly to the policy-selected general, explore, verifier, or scout fallback as applicable; do not invent a model or launder permissions. If no Work Unit exists because Task-Orchestrator startup failed before leaf delegation, this is Task-level recovery and no Work Unit may be fabricated.
